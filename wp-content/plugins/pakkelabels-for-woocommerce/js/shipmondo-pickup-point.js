jQuery(document).ready(function($) {
    ...
    $(document).on('shipmondo_zipcode_field_updated', zipcode_fields, function() {
        ...
        var error_text = wrapper.find('.shipmondo_zicode_error_text');
        if(isZipcodeValid($(this).val())) {
    ...
    function getPickupPoints(agent, zipcode, display_type, element, callback) {
        ...
        if(typeof pickup_points_html[agent] !== 'undefined' &&
           typeof pickup_points_html[agent][country] !== 'undefined' &&
           typeof pickup_points_html[agent][country][zipcode] !== 'undefined' &&
           typeof pickup_points_html[agent][country][zipcode][display_type] !== 'undefined') {
            return callback(element, pickup_points_html[agent][country][zipcode][display_type]);
        }

        // Get pickup points data
        $.post(shipmondo.ajax_url, {
            'action': 'shipmondo_get_pickup_points',
            ...
            if(typeof pickup_points_html[agent] === 'undefined') {
                pickup_points_html[agent] = [];
            }
            if(typeof pickup_points_html[agent][country] === 'undefined') {
                pickup_points_html[agent][country] = [];
            }
            if(typeof pickup_points_html[agent][country][zipcode] === 'undefined') {
                pickup_points_html[agent][country][zipcode] = [];
            }
            pickup_points_html[agent][country][zipcode][display_type] = result;
    ...
    function setShopHTML(index, shop, wrapper) {
        ...
        wrapper.find('.shipmondo-shop-id').html(shop.id_string);
    ...
    $(document).on('click', '.shipmondo_select_button[data-selection-type="modal"]', function(e) {
        e.stopPropagation();
        getPickupPointsModal($(this));
    });

    // Open modal and get the pickup points
    function getPickupPointsModal(element) {
        ...
        getPickupPoints(agent, zipcode, 'modal', element, function(element, html) {
            var wrapper = getWrapper(element);
            if(html === false) {
    ...
    $(document).on('click', '.shipmondo_select_button[data-selection-type="dropdown"]', function(e) {
        ...
        e.stopPropagation();
        getPickupPointsDropdown($(this));
    });

    // Open dropdown and get the pickup points
    function getPickupPointsDropdown(element) {
        var wrapper = getWrapper(element);
        var dropdown = wrapper.find('.shipmondo_pickup_point_selector_dropdown');
        ...
        getPickupPoints(agent, zipcode, 'dropdown', element, function(element, html) {
            var wrapper = getWrapper(element);
            var dropdown = wrapper.find('.shipmondo_pickup_point_selector_dropdown');
    ...
    $(document).on('click', function(e) {
        var dropdown = $('.shipmondo_pickup_point_selector_dropdown:not(".shipmondo-hidden")');
