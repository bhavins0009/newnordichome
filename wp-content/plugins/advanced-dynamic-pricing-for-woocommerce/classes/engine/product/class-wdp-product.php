...
//		$this->wc_product->set_price( $new_price );
	 ...
	 * @return false|string
	 */
	public function get_price_html( $qty = 1 ) {
		if ( ! $this->are_rules_applied() ) {
			return $this->get_wc_price_html( $qty );
		}
		...
		if ( $this->is_variable() || $this->is_grouped() ) {
			...
			if ( $this->is_range_correct() ) {
				$price_html = wc_format_price_range( $this->get_min_price() * $qty, $this->get_max_price() * $qty ) . $this->get_price_suffix();
				...
				if ( $this->is_on_wdp_sale() ) {
					$price_html = apply_filters(
						...
						wc_format_sale_price( wc_price( $this->get_price() * $qty ), wc_price( $this->get_min_price() * $qty ) ) . $this->get_price_suffix(),
						$this->get_price(),
						$this->get_min_price(),
						$this->wc_product,
						$qty
					...
					$price_html = wc_price( $this->get_new_price() * $qty ) . $this->get_price_suffix();
				}
			} else {
				// min price greater than max price
				return false;
			}
			$price_html = apply_filters( 'wdp_woocommerce_variable_price_html', $price_html, $this, $this->wc_product, $qty );
		} else {
			if ( $this->is_on_wdp_sale() ) {
				$price_html = wc_format_sale_price( wc_price( $this->get_price() * $qty ), wc_price( $this->get_new_price() * $qty ) ) . $this->get_price_suffix();
			} else {
				$price_html = wc_price( $this->get_new_price() * $qty ) . $this->get_price_suffix();
			}
			$price_html = apply_filters( 'wdp_woocommerce_discounted_price_html', $price_html, $this->get_price(), $this->get_new_price(), $this->wc_product, $qty );
		}
