...
class WDP_Price_Display {
	...
	public function init_while_doing_cron() {
		$this->apply_calc();
...
//		add_filter( 'woocommerce_variable_price_html', array( $this, 'hook_get_price_html' ), 100, 2 );
	...
	public function hook_get_price_html( $price_html, $product ) {
		...
		$bulk_price = $this->maybe_replace_with_bulk_price( $wdp_product );
		if ( ! is_null( $bulk_price ) ) {
			return $bulk_price;
		}
		...
		if ( $product_price_html = $this->get_product_price_html_without_pricing( $wdp_product ) ) {
...
//		}
	 ...
	 * @return array
	 */
	public function get_product_price_and_subtotal_html( $product, $qty, $args ) {
		...
		if ( ! $allow_modify ) {
			return array(
				'price_html'    => $this->get_product_price_html_without_pricing( $product ),
				'subtotal_html' => $this->get_product_price_html_without_pricing( $product, $qty ),
			);
		}

		$new_product = $this->process_product( $product, $qty );
		if ( ! is_null( $new_product ) ) {
			if ($new_product->are_rules_applied()) {
				if ( $new_product->is_on_wdp_sale() ) {
					if ( $allow_striked ) {
						$price_html = wc_format_sale_price( $new_product->get_price(), $new_product->get_new_price() ) . $new_product->get_price_suffix();
						$subtotal_html = wc_format_sale_price( $new_product->get_price() * $qty, $new_product->get_new_price() * $qty ) . $new_product->get_price_suffix();
					} else {
						$price_html = wc_price( $new_product->get_new_price() ) . $new_product->get_price_suffix();
						$subtotal_html = wc_price( $new_product->get_new_price() * $qty ) . $new_product->get_price_suffix();
					}
				} else {
					$price_html = wc_price( $new_product->get_price() ) . $new_product->get_price_suffix();
					$subtotal_html = wc_price( $new_product->get_price() * $qty ) . $new_product->get_price_suffix();
				}
			} else {
				$price_html = $this->get_product_price_html_without_pricing( $new_product );
				$subtotal_html = $this->get_product_price_html_without_pricing( $new_product, $qty );
			}

			$price_html = apply_filters( 'wdp_get_discounted_price_html', $price_html, $new_product->get_price(), $new_product->get_new_price(), $new_product );
			$subtotal_html = apply_filters( 'wdp_get_discounted_subtotal_html', $subtotal_html, $new_product->get_price(), $new_product->get_new_price(), $new_product, $qty );
		} else {
			$price_html = $this->get_product_price_html_without_pricing( $product );
			$subtotal_html = $this->get_product_price_html_without_pricing( $product );
		}
	 ...
	 * @return array
	 */
	public function get_prices_html_depends_on_page( $product, $qty, $page_data ) {
		...
		return $this->get_product_price_and_subtotal_html( $product, $qty, $args );
	}
	 ...
	 * @return string|void
	 */
	protected function maybe_replace_with_bulk_price( $wdp_product ) {
		...
		if ( WDP_Frontend::is_catalog_view() && $this->get_option( 'replace_price_with_min_bulk_price' ) && $wdp_product->is_affected_by_bulk() && $apply_not_fully_affected ) {
			$price_html = htmlspecialchars_decode($this->get_option( 'replace_price_with_min_bulk_price_template' ));
