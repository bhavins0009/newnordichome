...
class WDP_Cart {
	...
	public function __construct( $context, $wc_cart = false ) {
		...
		if ( $this->wc_cart instanceof WC_Cart ) {
			foreach ( $this->wc_cart->get_cart_contents() as $wc_cart_item ) {
				...
				$product = WDP_Object_Cache::get_instance()->get_wc_product( $product );
	 ...
	 * @return boolean
	 */
	private function get_is_on_sale( $product, $item_meta = array() ) {
		return apply_filters( "wdp_get_product_is_on_sale", $product->is_on_sale( $this->context->get_option('initial_price_context') ), $product, $item_meta );
	}
	 ...
	 * @return float
	 */
	private function get_regular_price( $product, $item_meta = array() ) {
		return apply_filters( "wdp_get_product_regular_price", $product->get_regular_price( $this->context->get_option('initial_price_context') ), $product, $item_meta );
	}
	 ...
	 * @return float
	 */
	private function get_sale_price( $product, $item_meta = array() ) {
		return apply_filters( "wdp_get_product_sale_price", $product->get_sale_price( $this->context->get_option('initial_price_context') ), $product, $item_meta );
	}
	 ...
	 * @return float
	 */
	private function get_price( $product, $item_meta = array() ) {
		return apply_filters( "wdp_get_product_initial_price", $product->get_price( $this->context->get_option('initial_price_context') ), $product, $item_meta );
	}
	...
	public function get_original_price( $product, $item_meta = array() ) {
		...
		if ( $this->get_is_on_sale( $product, $item_meta ) ) {
			if ( 'sale_price' === $price_mode || 'discount_sale' === $price_mode ) {
				$price = $this->get_sale_price( $product, $item_meta );
			} else {
				$price = $this->get_regular_price( $product, $item_meta );
			}
		} else {
			$price = $this->get_price( $product, $item_meta );
		}
	...
	public function add_product_to_calculate( $the_product, $qty = 1 ) {
		...
		$item = new WDP_Cart_Item( $hash, $original_price, $qty );
	...
	public function apply_to_wc_cart() {
		...
		foreach ( $this->items as $item ) {
			...
			$wdp_product->apply_initial_data( $this->get_item_initial_data( $item ) );
